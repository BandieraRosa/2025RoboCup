...
static const uint16_t ov5640_config[][2] =
{
...
{ 
    ...
    uint8_t state = 0x8F;
    dvp_sccb_send_data(OV5640_ADDR, 0x3000, 0x20);                   /* reset MCU */

    for (i = 0; i < sizeof(OV5640_AF_Config); i++)    /* 发送配置数组 */
    {
        dvp_sccb_send_data(OV5640_ADDR, addr, OV5640_AF_Config[i]);
        addr++;
    }

    dvp_sccb_send_data(OV5640_ADDR,0x3022, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3023, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3024, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3025, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3026, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3027, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3028, 0x00);
    dvp_sccb_send_data(OV5640_ADDR,0x3029, 0x7f);
    dvp_sccb_send_data(OV5640_ADDR,0x3000, 0x00); 
    i = 0;

    do
    {
        state = dvp_sccb_receive_data(OV5640_ADDR, 0x3029);
        msleep(5);
        i++;
...
uint8_t ov5640_focus_constant(void)
{
    ...
    dvp_sccb_send_data(OV5640_ADDR, 0x3023, 0x01);
    dvp_sccb_send_data(OV5640_ADDR, 0x3022, 0x08); /* 发送IDLE指令 */

    do 
    {
        temp = dvp_sccb_receive_data(OV5640_ADDR, 0x3023); 
        retry++;
    ...
    dvp_sccb_send_data(OV5640_ADDR, 0x3023, 0x01);
    dvp_sccb_send_data(OV5640_ADDR, 0x3022, 0x04); /* 发送持续对焦指令 */
    retry = 0;
...
void ov5640_rgb565_mode(void) 
{
    ...
    {
        dvp_sccb_send_data(OV5640_ADDR,ov5640_rgb565_reg_tbl[i][0], ov5640_rgb565_reg_tbl[i][1]); /* 发送配置数组 */
    }
...
static int ov5640_init(void)
{
    ...
    /* Check product ID */
    product_id = (dvp_sccb_receive_data(OV5640_ADDR, 0X300A) << 8) | dvp_sccb_receive_data(OV5640_ADDR, 0X300B);
    printf("ID:%x\n",product_id);
    if (product_id != OV5640_PID)
    {
        return 1;
    }

    /* Software reset */
    dvp_sccb_send_data(OV5640_ADDR, 0x3103, 0X11);
    dvp_sccb_send_data(OV5640_ADDR, 0X3008, 0X82);
    msleep(5);

    /* Initlize registers */
    for (index=0; index<(sizeof(ov5640_config)/sizeof(ov5640_config[0])); index++)
    {
        dvp_sccb_send_data(OV5640_ADDR, ov5640_config[index][0], ov5640_config[index][1]);
    }
...
static int ov5640_set_pixformat(pixformat_t format)
{
    return 0;
}
 ...
 * @retval      0, 操作成功
 */
static int ov5640_set_framesize(uint16_t width, uint16_t height)
{
    float ratio;
    uint16_t H_SIZE;
    uint16_t V_SIZE;
    ...
    ratio = (2592.f / 1944.f) / ((float)width / height);
    if (fabsf(ratio - 1) < 0.0001f)
    {
        H_SIZE = 1600;
        V_SIZE = 1200;
        OFFSET_X = (2592 - 1600) >> 1;
        OFFSET_Y = (1944 - 1200) >> 1;
    }
    else if (ratio > 1)
    {
        H_SIZE = (uint16_t)(1600 / ratio);
        V_SIZE = 1200;
        OFFSET_X = (2592 - H_SIZE) >> 1;
        OFFSET_Y = (1944 - 1200) >> 1;
    }
    else
    {
        H_SIZE = 1600;
        V_SIZE = (uint16_t)(1200 * ratio);
        OFFSET_X = (2592 - 1600) >> 1;
        OFFSET_Y = (1944 - V_SIZE) >> 1;
    }

    dvp_sccb_send_data(OV5640_ADDR, 0X3212, 0X03);      /* 开始组3 */

    /* 设置ISP窗口尺寸 */
    dvp_sccb_send_data(OV5640_ADDR, 0X3800, (uint8_t)(OFFSET_X >> 8));
    dvp_sccb_send_data(OV5640_ADDR, 0X3801, (uint8_t)(OFFSET_X & 0xFF));
    dvp_sccb_send_data(OV5640_ADDR, 0X3802, (uint8_t)(OFFSET_Y >> 8));
    dvp_sccb_send_data(OV5640_ADDR, 0X3803, (uint8_t)(OFFSET_Y & 0xFF));
    dvp_sccb_send_data(OV5640_ADDR, 0X3804, (uint8_t)((H_SIZE + OFFSET_X - 1) >> 8));
    dvp_sccb_send_data(OV5640_ADDR, 0X3805, (uint8_t)((H_SIZE + OFFSET_X - 1) & 0xFF));
    dvp_sccb_send_data(OV5640_ADDR, 0X3806, (uint8_t)((V_SIZE + OFFSET_Y - 1) >> 8));
    dvp_sccb_send_data(OV5640_ADDR, 0X3807, (uint8_t)((V_SIZE + OFFSET_Y - 1) & 0xFF));

    /* 以下设置决定实际输出尺寸(带缩放) */
    dvp_sccb_send_data(OV5640_ADDR, 0x3808, (uint8_t)(width >> 8));    /* 设置实际输出宽度高字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x3809, (uint8_t)(width & 0xff));  /* 设置实际输出宽度低字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x380a, (uint8_t)(height >> 8));   /* 设置实际输出高度高字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x380b, (uint8_t)(height & 0xff)); /* 设置实际输出高度低字节 */
    /* 以下设置决定输出尺寸在ISP上面的取图范围 */
    /* 会自动缩放成输出尺寸 */
    dvp_sccb_send_data(OV5640_ADDR, 0x3810,(uint8_t)(4 >> 8));      /* 设置X offset高字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x3811,(uint8_t)(4 & 0xff));    /* 设置X offset低字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x3812,(uint8_t)(3 >> 8));      /* 设置Y offset高字节 */
    dvp_sccb_send_data(OV5640_ADDR, 0x3813,(uint8_t)(3 & 0xff));    /* 设置Y offset低字节 */

    dvp_sccb_send_data(OV5640_ADDR, 0X3212, 0X13);
    dvp_sccb_send_data(OV5640_ADDR, 0X3212, 0Xa3);
 ...
 * @retval      0, 操作成功
 */
static int ov5640_set_hmirror(uint8_t enable)
{
    if (enable != 0)
    {
        dvp_sccb_send_data(OV5640_ADDR, 0x3821, 0x06);
    }
    else
    {
        dvp_sccb_send_data(OV5640_ADDR, 0x3821, 0x00);
    }
    return 0;
}
 ...
 * @retval      0, 操作成功
 */
static int ov5640_set_vflip(uint8_t enable)
{
    if (enable != 0)
    {
        dvp_sccb_send_data(OV5640_ADDR, 0x3820, 0x40);
    }
    else
    {
        dvp_sccb_send_data(OV5640_ADDR, 0x3820, 0x46);
    }
    return 0;
}

/**
 * @brief       OV5640 设置补光灯开关
...
const camera_sensor_t camera_ov5640 =
{
    .reg_len = 16,
    .xclk_rate = 24000000,
    .init = ov5640_init,
    .set_pixformat = ov5640_set_pixformat,
    .set_framesize = ov5640_set_framesize,
    .set_hmirror = ov5640_set_hmirror,
    .set_vflip = ov5640_set_vflip,
    .set_light = ov5640_set_light,
};
